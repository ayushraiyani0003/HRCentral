...
export const fetchDesignationById = createAsyncThunk(
  ...
  async (designationId, { rejectWithValue }) => {
    try {
      ...
      if (!response.success) {
        return rejectWithValue(response.error || "Failed to fetch designation");
      }
      return response.data;
    } catch (error) {
      return rejectWithValue(error.message || "Failed to fetch designation");
    }
  }
);
...
export const updateDesignation = createAsyncThunk(
  "designations/update",
  async ({ designationId, designationData }, { rejectWithValue, dispatch }) => {
    try {
      // Validate data first
      const validation = designationService.validateData(designationData);
      if (!validation.isValid) {
...
export const searchDesignations = createAsyncThunk(
  ...
  async (searchTerm, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || "Failed to search designations");
    }
  }
);
...
export const fetchDesignationsByDepartment = createAsyncThunk(
  ...
  async (department, { rejectWithValue }) => {
    try {
      const results = await designationService.getByDepartment(department);
...
const designationSlice = createSlice({
  ...
  reducers: {
    ...
    updateFilters: (state, action) => {
      ...
      if (state.filters.level !== null && state.filters.level !== undefined) {
        filtered = filtered.filter(
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Fetch by department
      .addCase(fetchDesignationsByDepartment.fulfilled, (state, action) => {
      ...
      .addCase(fetchDesignationsSortedByLevel.fulfilled, (state, action) => {
...
export const selectDesignationById = (state, id) =>
  state.designations.designations.find((designation) => designation.id === id);
