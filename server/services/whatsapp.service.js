...
const setupContacts = (contacts, settings) => {
    ...
    // Calculate batches
    sessionState.totalBatches = Math.ceil(
        contacts.length / sessionState.batchSize
    );
...
const processBatches = async () => {
    ...
    if (
        currentBatch > sessionState.totalBatches ||
        !sessionState.processingActive
    ) {
        return;
    }
    ...
    const batchEnd = Math.min(
        batchStart + sessionState.batchSize,
        sessionState.contacts.length
    );
    ...
    for (let i = index; i < batchEnd; i++) {
        ...
        sessionState.overallProgress = Math.round(
            (sessionState.currentIndex / sessionState.contacts.length) * 100
        );
        sessionState.batchProgress = Math.round(
            ((i - batchStart + 1) / (batchEnd - batchStart)) * 100
        );
    ...
    const randomBatchDelay = createNaturalDelay(
        sessionState.batchInterval,
        "batch"
    );
...
const sendPDF = async (contact) => {
    ...
    if (actualFileName !== expectedFileName) {
        throw new Error(
            `Filename mismatch: expected "${expectedFileName}", got "${actualFileName}"`
        );
    }
    ...
    if (cleanNumber.length !== 10) {
        throw new Error(
            `Invalid phone number (must be 10 digits): ${contact.phoneNo}`
        );
    }
    ...
    try {
        ...
        // Add timeout to prevent hanging on serialize errors
        const sendPromise = whatsappClient.sendMessage(
            `91${cleanNumber}@c.us`,
            media,
            {
                caption,
        ...
        try {
            ...
            if (timeoutError.message === "Send timeout") {
                // Message likely sent but response timed out
                console.log(
                    `Send timeout for ${contact.phoneNo} - message likely sent successfully`
                );
                return true;
            }
            throw timeoutError;
        ...
        // Check if this is the serialize error (message was actually sent successfully)
        if (
            error.message &&
            ...
            console.log(
                `PDF sent successfully to ${contact.phoneNo} (ignoring serialize error)`
            );
            return true;
        }

        // For other errors, check if it's a known whatsapp-web.js issue where message was sent
        if (
            error.message &&
            (error.message.includes("getMessageModel") ||
                error.message.includes("Evaluation failed: TypeError") ||
                error.message.includes(
                    "pptr://__puppeteer_evaluation_script__"
                ))
        ) {
            console.log(
                `PDF likely sent successfully to ${contact.phoneNo} (ignoring evaluation error)`
            );
            return true;
        }
